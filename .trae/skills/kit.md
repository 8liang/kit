...
1. **timepass (时间流逝与跨越检查)**
   ...
   - 用法：实现相应的时间流逝接口，使用 `timepass.Advance(t)` 触发检查。支持自定义周起始日（默认周一）。
...
5. **weighted (权重抽取系统)**
   - 场景：游戏或营销活动中的物品权重抽取，或其他按权重随机选择的场景。
   - 用法：初始化抽取器 (`weighted.NewSelector`)，添加带权重的物品，调用 `Draw(opts...)` (Option 模式) 抽取。
