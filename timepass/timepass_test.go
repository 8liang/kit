...
func (s *stubTimeTunnel) LastTouchedAt() time.Time {
	return s.touchedAt
}
...
func (s *stubTimeTunnel) OnHour() {
	s.Hour = true
}

func (s *stubTimeTunnel) OnDay() {
	s.Day = true
}

func (s *stubTimeTunnel) OnWeek() {
	s.Week = true
}

func (s *stubTimeTunnel) OnMonth() {
	s.Month = true
}
...
func TestTunnel(t *testing.T) {
	...
	Advance(tt, WithWeekStartsAt(carbon.Monday), WithCurrentTime(current))
	assert.True(t, tt.Week, "week should be crossed")
	assert.Equal(t, current.Unix(), tt.LastTouchedAt().Unix(), "touchedAt should be updated")

	tm := time.Now()
	stub := &stubTimeTunnel{touchedAt: tm.Add(-time.Second * 30)}
	Advance(stub, WithWeekStartsAt(carbon.Monday))
	assert.True(t, stub.Minute, "hour should be crossed")
	assert.False(t, stub.Hour, "hour should not be crossed")
	...
	assert.Equal(t, time.Now().Unix(), stub.LastTouchedAt().Unix(), "touchedAt should be updated")
